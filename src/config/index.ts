...
import { createMetaClient, type CredentialSummary, type EnvVarSummary, type MemberSummary, type MetaClient } from "../metaClient.js";
...
type HubData = {
  status: Awaited<ReturnType<MetaClient["getFactoryStatus"]>>;
  env: EnvVarSummary[];
...
export function registerConfigHandlers(app: MetaAppLike, dependencies: ConfigDependencies): void {
  ...
  app.action?.(ACTION_IDS.toggleCloud, async (args) => {
    ...
    try {
      ...
      const currentEnabled = hub.status.cloud_enabled;

      await client.updateCloud(metadata.projectId, { enabled: !currentEnabled });
  ...
  app.view(CFG_CREDS_CALLBACK_ID, async (args) => {
    try {
      ...
      const provider = args.body.view.state.values[CFG_CREDS_FIELDS.provider.blockId]?.[CFG_CREDS_FIELDS.provider.actionId]?.selected_option?.value ?? "";
...
export function buildConfigHubBlocks(
  hub: HubData,
  metadata: ConfigModalMetadata & { channelId?: string },
) {
  ...
  const credsSummary = hub.credentials.length > 0
  ...
  const blocks = [
    ...
    section(`*Credentials*  ${credsSummary}`, {
      accessory: button({
        actionId: ACTION_IDS.manageCredentials,
        text: "Manage",
        value: actionValue,
      }),
    }),
